// deepak start - new file: Muslim Population feature (admin content manager)
// Mirrors components/dashboard/content.tsx but scoped to the three Muslim
// Population page levels (Home/State/District) and pointed at the separate
// getMuslimPopulationContent/saveMuslimPopulationContent + getMuslimPopulationStates/
// getMuslimPopulationDistricts helpers, so it never touches the main site's
// Content collection or States/Districts collection.
"use client";
import { useState, useEffect } from "react";
import {
  getMuslimPopulationContent,
  saveMuslimPopulationContent,
...
function MuslimPopulationContentPage() {
  ...
  useEffect(() => {
    ...
    getMuslimPopulationDistricts({ state_slug: selectedState.state_slug })
  ...
  async function fetchAndSetContent(
    ...
    setSuccess("");
    try {
      const data = await getMuslimPopulationContent(tab, params);
      if (data?.error) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await saveMuslimPopulationContent(
        activeTab,
        forms[activeTab],
        slugParams,
      );
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-bold">
        Muslim Population Content Manager
      </h1>
      <p className="mb-4 text-sm text-gray-500">
        Manage the wording shown on the Muslim Population pages (
        <code>/muslimpopulation</code>, state and district pages). Separate from
...
export default MuslimPopulationContentPage;
// deepak end - new file
