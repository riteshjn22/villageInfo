// deepak start - new file: Christian Population feature (admin content manager)
// Mirrors components/dashboard/content.tsx but scoped to the three Christian
// Population page levels (Home/State/District) and pointed at the separate
// getChristianPopulationContent/saveChristianPopulationContent + getChristianPopulationStates/
// getChristianPopulationDistricts helpers, so it never touches the main site's
// Content collection or States/Districts collection.
"use client";
import { useState, useEffect } from "react";
import {
  getChristianPopulationContent,
  saveChristianPopulationContent,
...
function ChristianPopulationContentPage() {
  ...
  useEffect(() => {
    ...
    getChristianPopulationDistricts({ state_slug: selectedState.state_slug })
  ...
  async function fetchAndSetContent(
    ...
    setSuccess("");
    try {
      const data = await getChristianPopulationContent(tab, params);
      if (data?.error) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await saveChristianPopulationContent(
        activeTab,
        forms[activeTab],
        slugParams,
      );
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-bold">
        Christian Population Content Manager
      </h1>
      <p className="mb-4 text-sm text-gray-500">
        Manage the wording shown on the Christian Population pages (
        <code>/christianpopulation</code>, state and district pages). Separate
        from the main Content Manager.
      </p>
...
export default ChristianPopulationContentPage;
// deepak end - new file
