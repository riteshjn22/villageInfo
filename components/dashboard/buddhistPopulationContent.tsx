// deepak start - new file: Buddhist Population feature (admin content manager)
// Mirrors components/dashboard/content.tsx but scoped to the three Buddhist
// Population page levels (Home/State/District) and pointed at the separate
// getBuddhistPopulationContent/saveBuddhistPopulationContent + getBuddhistPopulationStates/
// getBuddhistPopulationDistricts helpers, so it never touches the main site's
// Content collection or States/Districts collection.
"use client";
import { useState, useEffect } from "react";
import {
  getBuddhistPopulationContent,
  saveBuddhistPopulationContent,
...
function BuddhistPopulationContentPage() {
  ...
  useEffect(() => {
    ...
    getBuddhistPopulationDistricts({ state_slug: selectedState.state_slug })
  ...
  async function fetchAndSetContent(
    ...
    setSuccess("");
    try {
      const data = await getBuddhistPopulationContent(tab, params);
      if (data?.error) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await saveBuddhistPopulationContent(
        activeTab,
        forms[activeTab],
        slugParams,
      );
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-bold">
        Buddhist Population Content Manager
      </h1>
      <p className="mb-4 text-sm text-gray-500">
        Manage the wording shown on the Buddhist Population pages (
        <code>/buddhistpopulation</code>, state and district pages). Separate
...
export default BuddhistPopulationContentPage;
// deepak end - new file
