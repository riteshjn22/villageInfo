// deepak start - new file: Hindu Population feature (admin content manager)
// Mirrors components/dashboard/content.tsx but scoped to the three Hindu
// Population page levels (Home/State/District) and pointed at the separate
// getHinduPopulationContent/saveHinduPopulationContent + getHinduPopulationStates/
// getHinduPopulationDistricts helpers, so it never touches the main site's
// Content collection or States/Districts collection.
"use client";
import { useState, useEffect } from "react";
import {
  getHinduPopulationContent,
  saveHinduPopulationContent,
...
function HinduPopulationContentPage() {
  ...
  useEffect(() => {
    ...
    getHinduPopulationDistricts({ state_slug: selectedState.state_slug })
  ...
  useEffect(() => {
    if (activeTab !== "District" || !selectedState || !selectedDistrict)
      return;
  ...
  async function fetchAndSetContent(
    ...
    setSuccess("");
    try {
      const data = await getHinduPopulationContent(tab, params);
      if (data?.error) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await saveHinduPopulationContent(
        activeTab,
        forms[activeTab],
        slugParams,
      );
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-bold">
        Hindu Population Content Manager
      </h1>
      <p className="mb-4 text-sm text-gray-500">
        Manage the wording shown on the Hindu Population pages (
        <code>/hindupopulation</code>, state and district pages). Separate
          ...
          <div className="flex flex-col gap-1">
            ...
            {statesLoading ? (
              ...
              <select
                ...
                onChange={(e) => {
                  setSelectedState(
                    states.find((s) => s.state_slug === e.target.value) ??
                      null,
                  );
                  setSuccess("");
                }}
...
export default HinduPopulationContentPage;
// deepak end - new file
