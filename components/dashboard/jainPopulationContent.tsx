// deepak start - new file: Jain Population feature (admin content manager)
// Mirrors components/dashboard/content.tsx but scoped to the three Jain
// Population page levels (Home/State/District) and pointed at the separate
// getJainPopulationContent/saveJainPopulationContent + getJainPopulationStates/
// getJainPopulationDistricts helpers, so it never touches the main site's
// Content collection or States/Districts collection.
"use client";
import { useState, useEffect } from "react";
import {
  getJainPopulationContent,
  saveJainPopulationContent,
...
function JainPopulationContentPage() {
  ...
  useEffect(() => {
    ...
    getJainPopulationDistricts({ state_slug: selectedState.state_slug })
  ...
  async function fetchAndSetContent(
    ...
    setSuccess("");
    try {
      const data = await getJainPopulationContent(tab, params);
      if (data?.error) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await saveJainPopulationContent(
        activeTab,
        forms[activeTab],
        slugParams,
      );
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-bold">
        Jain Population Content Manager
      </h1>
      <p className="mb-4 text-sm text-gray-500">
        Manage the wording shown on the Jain Population pages (
        <code>/jainpopulation</code>, state and district pages). Separate from
...
export default JainPopulationContentPage;
// deepak end - new file
