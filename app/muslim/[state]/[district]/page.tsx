// deepak start - new file: Muslim Population feature (district-level page)
...
// MuslimPopulationDistrict via getMuslimPopulationDistricts().
...
const getCachedDistrict = cache((state: string, district: string) =>
  getMuslimPopulationDistricts({ state_slug: state, district_slug: district }),
);
...
const getCachedDistrictContent = cache((state: string, district: string) =>
  getMuslimPopulationContent("district", {
    state_slug: state,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    ...
    data?.seo_title ||
    `Muslim Population in ${data?.district ?? district}, ${data?.state ?? state} – Muslim Sex Ratio`;
  const description =
    (hasContent && content.description) ||
    data?.seo_description ||
    `Muslim population, Muslim population percentage and Muslim sex ratio data for ${data?.district ?? district}, ${data?.state ?? state}.`;

  return {
    title,
    description,
    alternates: { canonical: `${HOST}/muslim/${state}/${district}` },
    openGraph: {
      title,
      description,
      url: `${HOST}/muslim/${state}/${district}`,
    },
  };
}

export default async function MuslimPopulationDistrictPage({ params }: Props) {
  const { state, district } = await params;
...
`,
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@id": `${HOST}/muslim/${state}/${district}/`,
            name: districtName,
            description: `Muslim population data for ${districtName}, ${stateName} based on Census ${census_year}.`,
            url: `${HOST}/muslim/${state}/${district}/`,
            address: {
              "@type": "PostalAddress",
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 3,
                name: stateName,
                item: `${HOST}/muslim/${state}/`,
              },
              {
                "@type": "ListItem",
                position: 4,
                name: `Muslim Population in ${districtName}`,
                item: `${HOST}/muslim/${state}/${district}/`,
              },
            ],
          }),
        }}
      ...
      <div className="vp-wrap">
        <nav className="vp-breadcrumb" aria-label="Breadcrumb">
          <Link href="/muslim">Muslim Population</Link>
          <span>›</span>
          <Link href={`/muslim/${state}`}>{stateName}</Link>
          <span>›</span>
          <strong>Muslim Population in {districtName}</strong>
        </nav>
        ...
        <div className="vp-hero">
          <div className="vp-hero-top">
            <div>
              <h1 className="vp-h1">
                Muslim Population in {districtName}, {stateName} – Male, Female
                &amp; Sex Ratio
              </h1>
              <p className="vp-intro">
                As per <strong>Census {census_year}</strong>, {districtName}{" "}
                district in {stateName} has a Muslim population of{" "}
                <strong>{muslim_population}</strong>, accounting for{" "}
                <strong>{muslim_population_percent}%</strong> of the
                district&apos;s total population. The Muslim sex ratio is{" "}
        ...
        <div className="vp-layout">
          <main>
            <div className="vp-content-box">
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>Muslim Male &amp; Female Population in {districtName}</h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
                  <div className="vp-card-title">
                    Muslim Population by Gender
                  </div>
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>Urban &amp; Rural Muslim Population in {districtName}</h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
                  <div className="vp-card-title">
          ...
          <aside className="vp-sidebar">
            <div className="vp-sidecard vp-sidecard-note">
              ...
              <p>
                This page presents Muslim population figures for {districtName}{" "}
            ...
            <div className="vp-sidecard">
              <h3>Explore</h3>
              <Link className="vp-sidelink" href={`/muslim/${state}`}>
                Muslim Population in {stateName}
              </Link>
              <Link className="vp-sidelink" href="/muslim">
