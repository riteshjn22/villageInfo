// deepak start - new file: Christian Population feature (district-level page)
// Layout + wording ported from the uploaded template1 HTML file. Field
// placeholders like {christian_population[1]} are replaced with live data from
// ChristianPopulationDistrict via getChristianPopulationDistricts().
import type { Metadata } from "next";
import { notFound } from "next/navigation";
import Link from "next/link";
import { cache } from "react";
import {
  getChristianPopulationDistricts,
  getChristianPopulationContent,
} from "@/utils/common";
import { HOST } from "@/lib/constants/constants";
// deepak start - new code: optional managed content (title/description/top/
// bottom/blog) from the Christian Population admin Content tab
...
const getCachedDistrictContent = cache((state: string, district: string) =>
  getChristianPopulationContent("district", {
    state_slug: state,
    district_slug: district,
  }),
);
...
type DistrictData = {
  ...
  christian_population?: number | string;
  christian_population_percent?: number | string;
  christian_sex_ratio_percent?: number | string;
  total_population_christian_males?: number | string;
  total_population_christian_females?: number | string;
  urban_christian_population?: number | string;
  rural_christian_population?: number | string;
  urban_christian_population_percenatge?: number | string;
  rural_christian_population_percentage?: number | string;
  seo_title?: string;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    ...
    data?.seo_title ||
    `Christian Population in ${data?.district ?? district}, ${data?.state ?? state} – Christian Sex Ratio`;
  const description =
    (hasContent && content.description) ||
    data?.seo_description ||
    `Christian population, Christian population percentage and Christian sex ratio data for ${data?.district ?? district}, ${data?.state ?? state}.`;

  return {
    title,
    description,
    alternates: {
      canonical: `${HOST}/christianpopulation/${state}/${district}`,
    },
    openGraph: {
      title,
      description,
      url: `${HOST}/christianpopulation/${state}/${district}`,
    },
  };
}

export default async function ChristianPopulationDistrictPage({
  params,
}: Props) {
  const { state, district } = await params;
...
`,
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@id": `${HOST}/christianpopulation/${state}/${district}/`,
            name: districtName,
            description: `Christian population data for ${districtName}, ${stateName} based on Census ${census_year}.`,
            url: `${HOST}/christianpopulation/${state}/${district}/`,
            address: {
              "@type": "PostalAddress",
            ...
            containedInPlace: {
              "@type": ["Place", "AdministrativeArea"],
              name: stateName,
              url: `${HOST}/christianpopulation/${state}/`,
            },
            additionalProperty: [
              {
                "@type": "PropertyValue",
              ...
              },
              {
                ...
                value: String(christian_population),
              },
              {
                "@type": "PropertyValue",
                name: "Christian Population Percentage",
                value: `${christian_population_percent}%`,
              },
              {
                "@type": "PropertyValue",
                name: "Christian Male Population",
                value: String(total_population_christian_males),
              },
              {
                "@type": "PropertyValue",
                name: "Christian Female Population",
                value: String(total_population_christian_females),
              },
              {
                "@type": "PropertyValue",
                name: "Christian Sex Ratio",
                value: `${christian_sex_ratio_percent} females per 1,000 males`,
              },
              {
                "@type": "PropertyValue",
                name: "Urban Christian Population",
                value: String(urban_christian_population),
              },
              {
                "@type": "PropertyValue",
                name: "Rural Christian Population",
                value: String(rural_christian_population),
              },
            ],
          }),
        }}
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Christian Population",
                item: `${HOST}/christianpopulation/`,
              },
              {
                "@type": "ListItem",
                position: 3,
                name: stateName,
                item: `${HOST}/christianpopulation/${state}/`,
              },
              {
                "@type": "ListItem",
                position: 4,
                name: `Christian Population in ${districtName}`,
                item: `${HOST}/christianpopulation/${state}/${district}/`,
              },
            ],
          }),
        }}
      ...
      <div className="vp-wrap">
        <nav className="vp-breadcrumb" aria-label="Breadcrumb">
          <Link href="/christianpopulation">Christian Population</Link>
          <span>›</span>
          <Link href={`/christianpopulation/${state}`}>{stateName}</Link>
          <span>›</span>
          <strong>Christian Population in {districtName}</strong>
        </nav>
        ...
        <div className="vp-hero">
          <div className="vp-hero-top">
            <div>
              <h1 className="vp-h1">
                Christian Population in {districtName}, {stateName} – Male,
                Female &amp; Sex Ratio
              </h1>
              <p className="vp-intro">
                As per <strong>Census {census_year}</strong>, {districtName}{" "}
                district in {stateName} has a Christian population of{" "}
                <strong>{christian_population}</strong>, accounting for{" "}
                <strong>{christian_population_percent}%</strong> of the
                district&apos;s total population. The Christian sex ratio is{" "}
                <strong>{christian_sex_ratio_percent}</strong> females per 1,000
                Christian males.
              </p>
          ...
          <div className="vp-stats">
            ...
            <div className="vp-stat">
              <div className="vp-stat-k">Christian Population</div>
              <div className="vp-stat-v">{christian_population}</div>
            </div>
            <div className="vp-stat">
              <div className="vp-stat-k">Christian Population %</div>
              <div className="vp-stat-v">{christian_population_percent}%</div>
            </div>
            <div className="vp-stat">
              <div className="vp-stat-k">Christian Sex Ratio</div>
              <div className="vp-stat-v">{christian_sex_ratio_percent}</div>
        ...
        <div className="vp-layout">
            ...
            <div className="vp-content-box">
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>Christian Population at a Glance</h2>
                  ...
                  <table className="vp-kv">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{christian_population}</td>
                      </tr>
                      <tr>
                        <th>Christian Population Percentage</th>
                        <td>{christian_population_percent}%</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>
                    Christian Male &amp; Female Population in {districtName}
                  </h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
                  <div className="vp-card-title">
                    Christian Population by Gender
                  </div>
                  <div className="vp-triblock">
                    <div className="vp-tri">
                      <div className="vp-tri-k">Christian Population</div>
                      <div className="vp-tri-v">{christian_population}</div>
                    </div>
                    <div className="vp-tri">
                      <div className="vp-tri-k">Christian Male</div>
                      <div className="vp-tri-v">
                        {total_population_christian_males}
                      </div>
                    </div>
                    <div className="vp-tri">
                      <div className="vp-tri-k">Christian Female</div>
                      <div className="vp-tri-v">
                        {total_population_christian_females}
                      </div>
                    </div>
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>
                    Urban &amp; Rural Christian Population in {districtName}
                  </h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
                  <div className="vp-card-title">
                    Christian Population by Residence
                  </div>
                  <div className="vp-duoblock">
                    <div className="vp-duo">
                      <div className="vp-duo-label">
                        Urban Christian Population
                      </div>
                      <div className="vp-duo-row">
                        <span>Population</span>
                        <b>{urban_christian_population}</b>
                      </div>
                      <div className="vp-duo-row">
                        <span>Share</span>
                        <b>
                    ...
                    <div className="vp-duo">
                      <div className="vp-duo-label">
                        Rural Christian Population
                      </div>
                      <div className="vp-duo-row">
                        <span>Population</span>
                        <b>{rural_christian_population}</b>
                      </div>
                      <div className="vp-duo-row">
                        <span>Share</span>
                        <b>
          ...
          <aside className="vp-sidebar">
            <div className="vp-sidecard vp-sidecard-note">
              ...
              <p>
                This page presents Christian population figures for{" "}
                {districtName} based on Census {census_year}. Christian
                population percentage represents the share of Christians in the
                total population. Christian sex ratio represents females per
                1,000 Christian males.
              </p>
            </div>
            ...
            <div className="vp-sidecard">
              <h3>Explore</h3>
              <Link
                className="vp-sidelink"
                href={`/christianpopulation/${state}`}
              >
                Christian Population in {stateName}
              </Link>
              <Link className="vp-sidelink" href="/christianpopulation">
