// deepak start - new file: Christian Population feature (state-level page)
...
// ChristianPopulationState via getChristianPopulationStates().
import type { Metadata } from "next";
import { notFound } from "next/navigation";
import Link from "next/link";
import { cache } from "react";
import {
  getChristianPopulationStates,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    ...
    stateData?.seo_title ||
    `Christian Population in ${stateData?.state ?? state} – Christian Sex Ratio`;
  const description =
    (hasContent && content.description) ||
    stateData?.seo_description ||
    `Christian population, Christian population percentage and Christian sex ratio data for ${stateData?.state ?? state}.`;

  return {
    title,
    description,
    alternates: { canonical: `${HOST}/christianpopulation/${state}` },
    openGraph: {
      title,
      description,
...
export default async function ChristianPopulationStatePage({ params }: Props) {
  const { state } = await params;

  const [stateData, districts, content] = await Promise.all([
    getCachedState(state) as Promise<StateData>,
    getChristianPopulationDistricts({ state_slug: state }) as Promise<
      DistrictListItem[]
    >,
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@id": `${HOST}/christianpopulation/${state}/`,
            name: stateName,
            alternateName: `${stateName} State`,
            description: `Christian population and Christian sex ratio data for ${stateName} based on Census ${census_year}.`,
            url: `${HOST}/christianpopulation/${state}/`,
            address: {
              "@type": "PostalAddress",
      ...
      <div className="vp-wrap">
        ...
        <div className="vp-hero">
          <div className="vp-hero-top">
            <div>
              <h1 className="vp-h1">
                Christian Population in {stateName} – Christian Male, Female
                &amp; Sex Ratio
              </h1>
              <p className="vp-intro">
                As per <strong>Census {census_year}</strong>, {stateName} has a
                Christian population of <strong>{christian_population}</strong>,
                accounting for <strong>{christian_population_percent}%</strong>{" "}
                of the state's total population. The Christian sex ratio is{" "}
        ...
        <div className="vp-layout">
          <main>
            <div className="vp-section" id="christian-population">
            ...
            <div className="vp-section" id="gender">
              <div className="vp-section-hd">
                <h2>Christian Male &amp; Female Population in {stateName}</h2>
                <div className="vp-hd-line"></div>
              </div>
              <div className="vp-card">
            ...
            <div className="vp-section" id="urban-rural">
              <div className="vp-section-hd">
                <h2>Urban &amp; Rural Christian Population</h2>
                <div className="vp-hd-line"></div>
              </div>
              <div className="vp-card">
                <div className="vp-card-title">
                    ...
                    <tbody>
                      {districtList.map((d) => (
                        <tr key={d.district_slug}>
                          <th>
                            <Link
                              href={`/christianpopulation/${state}/${d.district_slug}`}
                          ...
                          <td>
                            {d.christian_population ?? "—"} Christian population
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <aside className="vp-sidebar">
            <div className="vp-sidecard vp-sidecard-note">
              ...
              <p>
                This page presents Christian population figures for {stateName}{" "}
                based on Census {census_year}. Christian population percentage
                represents the share of Christians in the total population.
                Christian sex ratio represents females per 1,000 Christian
                males.
              </p>
            </div>
