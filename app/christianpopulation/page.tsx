// deepak start - new file: Christian Population feature (India-level index page)
// Placeholder list-of-states hub until a dedicated India-level template/Excel
// is supplied. Links into /christianpopulation/[state] for each state on file.
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = await getChristianPopulationContent("home", {});
  const title =
    (content && !content.error && content.title) ||
    `Christian Population in India by State | ${SITE_NAME}`;
  const description =
    (content && !content.error && content.description) ||
    "Browse Christian population, Christian population percentage and Christian sex ratio for every Indian state, based on Census data.";
  // deepak end - new code

  return {
    title,
    description,
    alternates: { canonical: `${HOST}/christianpopulation` },
    openGraph: { title, description, url: `${HOST}/christianpopulation` },
  };
}

export default async function ChristianPopulationIndexPage() {
  const states = (await getChristianPopulationStates()) as StateListItem[];
      ...
      <div className="flex w-full flex-col gap-4 rounded-2xl border border-gray-200 bg-linear-to-b from-slate-50 to-white p-4.5 shadow-[0_6px_18px_rgba(15,23,42,0.05)]">
        <h1 className="text-lg font-bold md:text-2xl">
          {(hasContent && content.title) ||
            "Christian Population in India â By State"}
        </h1>
        ...
        {hasContent && content.top_content ? (
          ...
          <p className="text-sm text-slate-700">
            {(hasContent && content.description) ||
              "Explore Christian population figures, Christian population percentage and Christian sex ratio for each Indian state based on Census data. Select a state below to view detailed district-wise Christian population data."}
          </p>
        )}
        {/* deepak end - new code */}
      </div>
      ...
      {stateList.length > 0 ? (
        <div className="mt-4 grid w-full grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
          {stateList.map((s) => (
            <Link
              key={s.state_slug}
              href={`/christianpopulation/${s.state_slug}`}
        ...
        <p className="mt-4 w-full text-sm text-slate-500">
          No Christian population data has been uploaded yet. Add states from
          the admin dashboard to populate this page.
        </p>
      )}
