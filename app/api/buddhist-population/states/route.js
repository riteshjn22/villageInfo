// deepak start - new file: Buddhist Population feature (state API)
// Mirrors app/api/states/route.js exactly, scoped to the BuddhistPopulationState
// collection so it never touches the existing /api/states data.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import BuddhistPopulationState from "@/lib/models/BuddhistPopulationState";
  ...
  try {
    ...
    if (hasParams) {
      ...
      if (limit && Object.keys(filter).length === 0) {
        const states = await BuddhistPopulationState.find()
          .sort({ state: 1 })
          .limit(limit)
      ...
      const state = await BuddhistPopulationState.findOne(filter).lean();
    ...
    console.error("GET /api/buddhist-population/states error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const existingState = await BuddhistPopulationState.findOne({
    ...
    const state = await BuddhistPopulationState.findOneAndUpdate(
      { state_id: body.state_id },
      { $set: body },
      {
    ...
    if (state_slug) {
      ...
      revalidatePath("/buddhistpopulation");
    }
    ...
    console.error("POST /api/buddhist-population/states error:", error);
