// deepak start - new file: Buddhist Population feature (district API)
// Mirrors app/api/districts/route.js exactly, scoped to the
// BuddhistPopulationDistrict collection so it never touches /api/districts data.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import BuddhistPopulationDistrict from "@/lib/models/BuddhistPopulationDistrict";
...
export async function GET(req) {
  try {
    ...
    if (searchParams.get("all") === "true") {
      const districts = await BuddhistPopulationDistrict.find()
    ...
    if (!state_slug && !district_slug) {
      const totalDistricts = await BuddhistPopulationDistrict.countDocuments();
    ...
    if (state_slug && district_slug) {
      const district = await BuddhistPopulationDistrict.findOne({
        state_slug,
        district_slug,
      }).lean();
    ...
    if (state_slug && searchParams.get("limit")) {
      ...
      const districts = await BuddhistPopulationDistrict.find({ state_slug })
        .sort({ buddhist_population: -1 })
        .limit(limit)
        .select("district district_slug state_slug buddhist_population")
        .lean();
    ...
    if (state_slug) {
      const districts = await BuddhistPopulationDistrict.find({ state_slug })
        .sort({ district: 1 })
        .select(
          "district district_slug state_slug buddhist_population buddhist_population_percent total_population",
        )
        .lean();
    ...
    console.error("GET /api/buddhist-population/districts error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const existingDistrict = await BuddhistPopulationDistrict.findOne({
    ...
    const district = await BuddhistPopulationDistrict.findOneAndUpdate(
    ...
    if (state_slug && district_slug) {
      revalidatePath(`/buddhistpopulation/${state_slug}/${district_slug}`);
      revalidatePath(`/buddhistpopulation/${state_slug}`);
    }
    ...
    console.error("POST /api/buddhist-population/districts error:", error);
