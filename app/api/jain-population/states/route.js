// deepak start - new file: Jain Population feature (state API)
// Mirrors app/api/states/route.js exactly, scoped to the JainPopulationState
// collection so it never touches the existing /api/states data.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import JainPopulationState from "@/lib/models/JainPopulationState";
  ...
  try {
    ...
    if (hasParams) {
      ...
      if (limit && Object.keys(filter).length === 0) {
        const states = await JainPopulationState.find()
          .sort({ state: 1 })
          .limit(limit)
      ...
      const state = await JainPopulationState.findOne(filter).lean();
    ...
    console.error("GET /api/jain-population/states error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const existingState = await JainPopulationState.findOne({
    ...
    const state = await JainPopulationState.findOneAndUpdate(
      { state_id: body.state_id },
      { $set: body },
      {
    ...
    console.error("POST /api/jain-population/states error:", error);
