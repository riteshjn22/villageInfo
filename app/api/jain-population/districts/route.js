// deepak start - new file: Jain Population feature (district API)
// Mirrors app/api/districts/route.js exactly, scoped to the
// JainPopulationDistrict collection so it never touches /api/districts data.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import JainPopulationDistrict from "@/lib/models/JainPopulationDistrict";
...
export async function GET(req) {
  try {
    ...
    if (searchParams.get("all") === "true") {
      const districts = await JainPopulationDistrict.find()
    ...
    if (!state_slug && !district_slug) {
      const totalDistricts = await JainPopulationDistrict.countDocuments();
    ...
    if (state_slug && district_slug) {
      const district = await JainPopulationDistrict.findOne({
        state_slug,
        district_slug,
      }).lean();
    ...
    if (state_slug && searchParams.get("limit")) {
      ...
      const districts = await JainPopulationDistrict.find({ state_slug })
        .sort({ jain_population: -1 })
        .limit(limit)
        .select("district district_slug state_slug jain_population")
        .lean();
    ...
    if (state_slug) {
      const districts = await JainPopulationDistrict.find({ state_slug })
        .sort({ district: 1 })
        .select(
          "district district_slug state_slug jain_population jain_population_percent total_population",
        )
        .lean();
    ...
    console.error("GET /api/jain-population/districts error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const existingDistrict = await JainPopulationDistrict.findOne({
    ...
    const district = await JainPopulationDistrict.findOneAndUpdate(
    ...
    if (state_slug && district_slug) {
      revalidatePath(`/jainpopulation/${state_slug}/${district_slug}`);
      revalidatePath(`/jainpopulation/${state_slug}`);
    }
    ...
    console.error("POST /api/jain-population/districts error:", error);
