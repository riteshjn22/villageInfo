// deepak start - new file: Sikh Population feature (district API)
// Mirrors app/api/districts/route.js exactly, scoped to the
// SikhPopulationDistrict collection so it never touches /api/districts data.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import SikhPopulationDistrict from "@/lib/models/SikhPopulationDistrict";
...
export async function GET(req) {
  try {
    ...
    if (searchParams.get("all") === "true") {
      const districts = await SikhPopulationDistrict.find()
    ...
    if (!state_slug && !district_slug) {
      const totalDistricts = await SikhPopulationDistrict.countDocuments();
    ...
    if (state_slug && district_slug) {
      const district = await SikhPopulationDistrict.findOne({
        state_slug,
        district_slug,
      }).lean();
    ...
    if (state_slug && searchParams.get("limit")) {
      ...
      const districts = await SikhPopulationDistrict.find({ state_slug })
        .sort({ sikh_population: -1 })
        .limit(limit)
        .select("district district_slug state_slug sikh_population")
        .lean();
    ...
    if (state_slug) {
      const districts = await SikhPopulationDistrict.find({ state_slug })
        .sort({ district: 1 })
        .select(
          "district district_slug state_slug sikh_population sikh_population_percent total_population",
        )
        .lean();
    ...
    console.error("GET /api/sikh-population/districts error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const existingDistrict = await SikhPopulationDistrict.findOne({
    ...
    const district = await SikhPopulationDistrict.findOneAndUpdate(
    ...
    if (state_slug && district_slug) {
      revalidatePath(`/sikhpopulation/${state_slug}/${district_slug}`);
      revalidatePath(`/sikhpopulation/${state_slug}`);
    }
    ...
    console.error("POST /api/sikh-population/districts error:", error);
