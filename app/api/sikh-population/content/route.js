// deepak start - new file: Sikh Population feature (content API)
// Mirrors app/api/content/route.js but reads/writes the separate
// SikhPopulationContent collection, scoped to home/state/district only.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import SikhPopulationContent from "@/lib/models/SikhPopulationContent";
import { CACHE_HEADERS } from "@/lib/constants/constants";

// GET /api/sikh-population/content?page_id=home
// GET /api/sikh-population/content?page_id=state&state_slug=andhra_pradesh
// GET /api/sikh-population/content?page_id=district&state_slug=andhra_pradesh&district_slug=visakhapatnam
export async function GET(req) {
  try {
    ...
    const content = await SikhPopulationContent.findOne(query).lean();
    ...
    console.error("GET /sikh-population/content error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const content = await SikhPopulationContent.findOneAndUpdate(
      filter,
      {
        $set: {
          title,
    ...
    console.error("POST /sikh-population/content error:", error);
    return NextResponse.json(
