// deepak start - new file: Muslim Population feature (state API)
// Mirrors app/api/states/route.js exactly, scoped to the MuslimPopulationState
// collection so it never touches the existing /api/states data.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import MuslimPopulationState from "@/lib/models/MuslimPopulationState";
  ...
  try {
    ...
    if (hasParams) {
      ...
      if (limit && Object.keys(filter).length === 0) {
        const states = await MuslimPopulationState.find()
          .sort({ state: 1 })
          .limit(limit)
      ...
      const state = await MuslimPopulationState.findOne(filter).lean();
    ...
    console.error("GET /api/muslim-population/states error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const existingState = await MuslimPopulationState.findOne({
    ...
    const state = await MuslimPopulationState.findOneAndUpdate(
      { state_id: body.state_id },
      { $set: body },
      {
    ...
    if (state_slug) {
      ...
      revalidatePath("/muslimpopulation");
    }
    ...
    console.error("POST /api/muslim-population/states error:", error);
