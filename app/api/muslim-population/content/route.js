// deepak start - new file: Muslim Population feature (content API)
// Mirrors app/api/content/route.js but reads/writes the separate
// MuslimPopulationContent collection, scoped to home/state/district only.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import MuslimPopulationContent from "@/lib/models/MuslimPopulationContent";
import { CACHE_HEADERS } from "@/lib/constants/constants";

// GET /api/muslim-population/content?page_id=home
// GET /api/muslim-population/content?page_id=state&state_slug=andhra_pradesh
// GET /api/muslim-population/content?page_id=district&state_slug=andhra_pradesh&district_slug=visakhapatnam
export async function GET(req) {
  try {
    ...
    const content = await MuslimPopulationContent.findOne(query).lean();
    ...
    console.error("GET /muslim-population/content error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const content = await MuslimPopulationContent.findOneAndUpdate(
      filter,
      {
        $set: {
          title,
    ...
    console.error("POST /muslim-population/content error:", error);
    return NextResponse.json(
