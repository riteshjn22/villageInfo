// deepak start - new file: Muslim Population feature (district API)
// Mirrors app/api/districts/route.js exactly, scoped to the
// MuslimPopulationDistrict collection so it never touches /api/districts data.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import MuslimPopulationDistrict from "@/lib/models/MuslimPopulationDistrict";
...
export async function GET(req) {
  try {
    ...
    if (searchParams.get("all") === "true") {
      const districts = await MuslimPopulationDistrict.find()
    ...
    if (!state_slug && !district_slug) {
      const totalDistricts = await MuslimPopulationDistrict.countDocuments();
    ...
    if (state_slug && district_slug) {
      const district = await MuslimPopulationDistrict.findOne({
        state_slug,
        district_slug,
      }).lean();
    ...
    if (state_slug && searchParams.get("limit")) {
      ...
      const districts = await MuslimPopulationDistrict.find({ state_slug })
        .sort({ muslim_population: -1 })
        .limit(limit)
        .select("district district_slug state_slug muslim_population")
        .lean();
    ...
    if (state_slug) {
      const districts = await MuslimPopulationDistrict.find({ state_slug })
        .sort({ district: 1 })
        .select(
          "district district_slug state_slug muslim_population muslim_population_percent total_population",
        )
        .lean();
    ...
    console.error("GET /api/muslim-population/districts error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const existingDistrict = await MuslimPopulationDistrict.findOne({
    ...
    const district = await MuslimPopulationDistrict.findOneAndUpdate(
    ...
    if (state_slug && district_slug) {
      revalidatePath(`/muslimpopulation/${state_slug}/${district_slug}`);
      revalidatePath(`/muslimpopulation/${state_slug}`);
    }
    ...
    console.error("POST /api/muslim-population/districts error:", error);
