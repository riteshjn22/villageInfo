// deepak start - new file: Christian Population feature (content API)
// Mirrors app/api/content/route.js but reads/writes the separate
// ChristianPopulationContent collection, scoped to home/state/district only.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import ChristianPopulationContent from "@/lib/models/ChristianPopulationContent";
import { CACHE_HEADERS } from "@/lib/constants/constants";

// GET /api/christian-population/content?page_id=home
// GET /api/christian-population/content?page_id=state&state_slug=andhra_pradesh
// GET /api/christian-population/content?page_id=district&state_slug=andhra_pradesh&district_slug=visakhapatnam
export async function GET(req) {
  try {
    ...
    const content = await ChristianPopulationContent.findOne(query).lean();
    ...
    console.error("GET /christian-population/content error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const content = await ChristianPopulationContent.findOneAndUpdate(
      filter,
      {
        $set: {
          title,
    ...
    console.error("POST /christian-population/content error:", error);
    return NextResponse.json(
