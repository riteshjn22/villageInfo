// deepak start - new file: Hindu Population feature (content API)
// Mirrors app/api/content/route.js but reads/writes the separate
// HinduPopulationContent collection, scoped to home/state/district only.
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
import HinduPopulationContent from "@/lib/models/HinduPopulationContent";
import { CACHE_HEADERS } from "@/lib/constants/constants";

// GET /api/hindu-population/content?page_id=home
// GET /api/hindu-population/content?page_id=state&state_slug=andhra_pradesh
// GET /api/hindu-population/content?page_id=district&state_slug=andhra_pradesh&district_slug=visakhapatnam
export async function GET(req) {
  try {
    ...
    const content = await HinduPopulationContent.findOne(query).lean();
    ...
    console.error("GET /hindu-population/content error:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    const content = await HinduPopulationContent.findOneAndUpdate(
      filter,
      {
        $set: {
          title,
    ...
    console.error("POST /hindu-population/content error:", error);
    return NextResponse.json(
