// deepak start - new file: Sikh Population feature (district-level page)
...
// SikhPopulationDistrict via getSikhPopulationDistricts().
...
const getCachedDistrict = cache((state: string, district: string) =>
  getSikhPopulationDistricts({ state_slug: state, district_slug: district }),
);
...
const getCachedDistrictContent = cache((state: string, district: string) =>
  getSikhPopulationContent("district", {
    state_slug: state,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    ...
    data?.seo_title ||
    `Sikh Population in ${data?.district ?? district}, ${data?.state ?? state} – Sikh Sex Ratio`;
  const description =
    (hasContent && content.description) ||
    data?.seo_description ||
    `Sikh population, Sikh population percentage and Sikh sex ratio data for ${data?.district ?? district}, ${data?.state ?? state}.`;

  return {
    title,
    description,
    alternates: { canonical: `${HOST}/sikhpopulation/${state}/${district}` },
    openGraph: {
      title,
      description,
      url: `${HOST}/sikhpopulation/${state}/${district}`,
    },
  };
}

export default async function SikhPopulationDistrictPage({ params }: Props) {
  const { state, district } = await params;
...
`,
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@id": `${HOST}/sikhpopulation/${state}/${district}/`,
            name: districtName,
            description: `Sikh population data for ${districtName}, ${stateName} based on Census ${census_year}.`,
            url: `${HOST}/sikhpopulation/${state}/${district}/`,
            address: {
              "@type": "PostalAddress",
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Sikh Population",
                item: `${HOST}/sikhpopulation/`,
              },
              {
                "@type": "ListItem",
                position: 3,
                name: stateName,
                item: `${HOST}/sikhpopulation/${state}/`,
              },
              {
                "@type": "ListItem",
                position: 4,
                name: `Sikh Population in ${districtName}`,
                item: `${HOST}/sikhpopulation/${state}/${district}/`,
              },
            ],
          }),
        }}
      ...
      <div className="vp-wrap">
        <nav className="vp-breadcrumb" aria-label="Breadcrumb">
          <Link href="/sikhpopulation">Sikh Population</Link>
          <span>›</span>
          <Link href={`/sikhpopulation/${state}`}>{stateName}</Link>
          <span>›</span>
          <strong>Sikh Population in {districtName}</strong>
        </nav>
        ...
        <div className="vp-hero">
          <div className="vp-hero-top">
            <div>
              <h1 className="vp-h1">
                Sikh Population in {districtName}, {stateName} – Male, Female
                &amp; Sex Ratio
              </h1>
              <p className="vp-intro">
                As per <strong>Census {census_year}</strong>, {districtName}{" "}
                district in {stateName} has a Sikh population of{" "}
                <strong>{sikh_population}</strong>, accounting for{" "}
                <strong>{sikh_population_percent}%</strong> of the
                district&apos;s total population. The Sikh sex ratio is{" "}
        ...
        <div className="vp-layout">
          <main>
            <div className="vp-content-box">
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>Sikh Male &amp; Female Population in {districtName}</h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>Urban &amp; Rural Sikh Population in {districtName}</h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
          ...
          <aside className="vp-sidebar">
            <div className="vp-sidecard vp-sidecard-note">
              ...
              <p>
                This page presents Sikh population figures for {districtName}{" "}
                based on Census {census_year}. Sikh population percentage
                represents the share of Sikhs in the total population. Sikh sex
                ratio represents females per 1,000 Sikh males.
              </p>
            </div>

            <div className="vp-sidecard">
              <h3>Explore</h3>
              <Link className="vp-sidelink" href={`/sikhpopulation/${state}`}>
                Sikh Population in {stateName}
              </Link>
              <Link className="vp-sidelink" href="/sikhpopulation">
