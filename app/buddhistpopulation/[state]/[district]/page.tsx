// deepak start - new file: Buddhist Population feature (district-level page)
...
// BuddhistPopulationDistrict via getBuddhistPopulationDistricts().
...
const getCachedDistrictContent = cache((state: string, district: string) =>
  getBuddhistPopulationContent("district", {
    state_slug: state,
    district_slug: district,
  }),
);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    ...
    data?.seo_title ||
    `Buddhist Population in ${data?.district ?? district}, ${data?.state ?? state} – Buddhist Sex Ratio`;
  const description =
    (hasContent && content.description) ||
    data?.seo_description ||
    `Buddhist population, Buddhist population percentage and Buddhist sex ratio data for ${data?.district ?? district}, ${data?.state ?? state}.`;

  return {
    title,
    description,
    alternates: {
      canonical: `${HOST}/buddhistpopulation/${state}/${district}`,
    },
    openGraph: {
      title,
      description,
      url: `${HOST}/buddhistpopulation/${state}/${district}`,
    },
  };
}

export default async function BuddhistPopulationDistrictPage({
  params,
}: Props) {
  const { state, district } = await params;
...
`,
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@id": `${HOST}/buddhistpopulation/${state}/${district}/`,
            name: districtName,
            description: `Buddhist population data for ${districtName}, ${stateName} based on Census ${census_year}.`,
            url: `${HOST}/buddhistpopulation/${state}/${district}/`,
            address: {
              "@type": "PostalAddress",
            ...
            containedInPlace: {
              "@type": ["Place", "AdministrativeArea"],
              name: stateName,
              url: `${HOST}/buddhistpopulation/${state}/`,
            },
            additionalProperty: [
              {
                "@type": "PropertyValue",
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Buddhist Population",
                item: `${HOST}/buddhistpopulation/`,
              },
              {
                "@type": "ListItem",
                position: 3,
                name: stateName,
                item: `${HOST}/buddhistpopulation/${state}/`,
              },
              {
                "@type": "ListItem",
                position: 4,
                name: `Buddhist Population in ${districtName}`,
                item: `${HOST}/buddhistpopulation/${state}/${district}/`,
              },
            ],
          }),
        }}
      ...
      <div className="vp-wrap">
        <nav className="vp-breadcrumb" aria-label="Breadcrumb">
          <Link href="/buddhistpopulation">Buddhist Population</Link>
          <span>›</span>
          <Link href={`/buddhistpopulation/${state}`}>{stateName}</Link>
          <span>›</span>
          <strong>Buddhist Population in {districtName}</strong>
        </nav>
        ...
        <div className="vp-hero">
          <div className="vp-hero-top">
            <div>
              <h1 className="vp-h1">
                Buddhist Population in {districtName}, {stateName} – Male,
                Female &amp; Sex Ratio
              </h1>
              <p className="vp-intro">
                As per <strong>Census {census_year}</strong>, {districtName}{" "}
                district in {stateName} has a Buddhist population of{" "}
                <strong>{buddhist_population}</strong>, accounting for{" "}
                <strong>{buddhist_population_percent}%</strong> of the
                district&apos;s total population. The Buddhist sex ratio is{" "}
        ...
        <div className="vp-layout">
          <main>
            <div className="vp-content-box">
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>
                    Buddhist Male &amp; Female Population in {districtName}
                  </h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
              ...
              <div className="vp-section">
                <div className="vp-section-hd">
                  <h2>
                    Urban &amp; Rural Buddhist Population in {districtName}
                  </h2>
                  <div className="vp-hd-line"></div>
                </div>
                <div className="vp-card">
          ...
          <aside className="vp-sidebar">
            <div className="vp-sidecard vp-sidecard-note">
              ...
              <p>
                This page presents Buddhist population figures for{" "}
                {districtName} based on Census {census_year}. Buddhist
                population percentage represents the share of Buddhists in the
                total population. Buddhist sex ratio represents females per
                1,000 Buddhist males.
              </p>
            </div>
            ...
            <div className="vp-sidecard">
              ...
              <Link
                ...
                href={`/buddhistpopulation/${state}`}
              >
                Buddhist Population in {stateName}
              </Link>
              <Link className="vp-sidelink" href="/buddhistpopulation">
